...
.TEMP 25.0
.OPTION ARTIST=2 INGOLD=2 PARHIER=LOCAL PSF=2 POST=2 ACCURATE NUMDGT=8 MEASOUT=1
...
** Output loads - 4 inverters per output bit for realistic fanout
** Output z_0<0> fanout
...
.MEAS TRAN v_z0_0 FIND v(z_0<0>) AT=99n
.MEAS TRAN v_z0_1 FIND v(z_0<1>) AT=99n
.MEAS TRAN v_z0_2 FIND v(z_0<2>) AT=99n

** Measure power
.MEAS TRAN power_avg AVG POWER FROM=50n TO=100n
...
if __name__ == '__main__':
    ...
    # Check if Neuron.sp exists
    neuron_file = 'Neuron_submod.sp'
    if not Path(neuron_file).exists():
        print(f"❌ Error: {neuron_file} not found!")
        print(f"Please make sure Neuron.sp is in the current directory.")
        sys.exit(1)
    ...
    results = verifier.run_verification(
        ...
        verbose=False,  # Set True to see all passing tests
        stop_on_fail=True  # Stop on first failure to keep files
    )
    
    if results is not None:  # Only save if verification completed
        # Save results
        verifier.save_results('verification_results.json')
        verifier.generate_truth_table('truth_table.txt')
